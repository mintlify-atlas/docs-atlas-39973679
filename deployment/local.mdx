---
title: "Run Giovanny Burger Locally for Development"
sidebarTitle: "Local"
...
|---|---|---|
| Desarrollo | `npm run dev` | Inicia el servidor Vite con HMR en `localhost:5173` |
| Producción | `npm run build` | Genera el bundle optimizado en `dist/` |
| Vista previa | `npm run preview` | Sirve el build de producción localmente |
| Linting | `npm run lint` | Ejecuta ESLint sobre el proyecto |
...
<Note>
  El carrito de compras persiste su estado en **localStorage** del navegador. Si necesitas resetear el carrito durante el desarrollo (por ejemplo, para probar el flujo desde cero), abre las **DevTools** del navegador → pestaña *Application* → *Storage* → *Local Storage* → `http://localhost:5173` y elimina la clave correspondiente al carrito. Al recargar la página, el estado quedará limpio.
</Note>
